...
describe("MockClock debounce", () => {
	test("debounced function is called once after delay", () => {
		const mockTime = new MockClock();
		const mockFn = jest.fn();
		const debouncedFn = mockTime.debounced(mockFn, 1000);
	...
	test("uses default delay when not specified", () => {
		const mockTime = new MockClock();
		const mockFn = jest.fn();
		const debouncedFn = mockTime.debounced(mockFn); // Default 500ms
